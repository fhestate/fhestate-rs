...
*   **2. Rust SDK**
    ...
    *   [`LocalCache`](#localcache) - Content-addressed ciphertext store
    *   [`StateTransition`](#statetransition) - Hash-chained FHE state machine
...
*   **3. Shielded Vault CLI**
    *   [Homomorphic commands](#shielded-vault-homomorphic-commands-fhe-cli) - Balance hashes, spending guard, DAO tally

*   **4. On-chain programs**
    *   [Deployed program IDs](#deployed-program-ids-devnet)
    *   [Integration binaries](#integration-binaries-devnet)
...
### Shielded Vault homomorphic commands (`fhe-cli`)

Implemented in `bin/fhe-cli/vault_ops.rs`. Each command prints **JSON to stdout** for backend and SDK integration. None of these submit Solana transactions directly.

#### `vault-transfer-hashes`

Homomorphic debit/credit between two balance ciphertexts.

```bash
fhe-cli vault-transfer-hashes \
  --sender-balance-uri local://<hash> \
  --receiver-balance-uri local://<hash> \
...
Use hashes with `execute_transfer_fhe` or `execute_transfer_fhe_tee`.

#### `vault-deposit-hash`

Post-shield balance after homomorphic deposit addition.

```bash
fhe-cli vault-deposit-hash --balance-uri local://<hash> --deposit-lamports <LAMPORTS>
```

**Output:** `{ "new_balance_hash", "new_balance_uri" }`

#### `vault-swap-hash`

Post-swap balance: `current - amount_in + amount_out` homomorphically.

```bash
fhe-cli vault-swap-hash \
  --current-balance-uri local://<hash> \
  --amount-in-lamports <IN> \
  --amount-out-lamports <OUT>
```

**Output:** `{ "new_balance_hash", "new_balance_uri" }` — feed `new_balance_hash` into `shielded_swap_proxy`.

#### `dao-tally-vote`

Accumulate encrypted vote into tally using `StateTransition::apply` with `ops::VOTE_TALLY`.

```bash
fhe-cli dao-tally-vote --tally-uri local://<hash> --vote-ciphertext-hex <HEX>
```

**Output:** `{ "new_state_hash", "new_state_uri" }`

#### `store-ciphertext`

Persist raw ciphertext bytes into `.fhe_cache`.

```bash
fhe-cli store-ciphertext --ciphertext-hex <HEX>
```
...
Decrypt `FheUint32` from cache URI or hex (requires `client_key.bin`).

```bash
fhe-cli decrypt-u32 --uri-or-hex local://<hash>
```
...
Spending guard: decrypts current daily spend ciphertext locally and compares `current + proposed <= limit`.

```bash
fhe-cli check-spending \
  --daily-spend-uri local://<hash> \
  --proposed-lamports <LAMPORTS> \
  --limit-lamports <LAMPORTS>
```
...
On-chain instruction layouts: [SHIELDED-VAULT-PROGRAM.md](./SHIELDED-VAULT-PROGRAM.md), [ARCHITECTURE.md](./ARCHITECTURE.md).

---

### Integration binaries (Devnet)

| Binary | `cargo` target | Purpose |
|--------|----------------|---------|
| `devnet_vault_flow` | `--bin devnet_vault_flow` | Shield, FHE transfer hashes, unshield |
| `devnet_vault_flow_tee` | `--bin devnet_vault_flow_tee` | TEE attestation + enclave registration |
| `devnet_vault_enclave_flow` | `--bin devnet_vault_enclave_flow` | Policy limits + `shielded_swap_proxy` E2E |
| `confidential_governance_flow` | `--bin confidential_governance_flow` | Treasury limit + governance hash updates |
| `close_registry` | `--bin close_registry` | Admin `close_registry` instruction |

All vault binaries use program `FuQzZCwPSRSVLT9gCgcft43a4RkapBJmSTC6CmdomeVQ`. Require `deploy-wallet.json` and `fhe_keys/`.
